...
GitHub Pages 会给样式和脚本设置 10 分钟缓存，所以 `index.html` / `lesson.html` 里引用
`style.css` 和 `app.js` 时带了版本号，例如 `assets/js/app.js?v=2`。
以后改动 CSS 或 JS 后，把这个数字加一（`v=3`、`v=4`……），
孩子打开网页时就会立刻看到新版本，不需要等缓存过期或手动刷新。
